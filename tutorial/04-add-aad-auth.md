...
1. Откройте **AuthenticationManager. h** и добавьте приведенный `#import` ниже оператор в начало файла.

    ```objc
    #import <MSGraphMSALAuthProvider/MSGraphMSALAuthProvider.h>
    ```

1. Добавьте указанную ниже строку в `@interface` объявление.

    ```objc
    - (MSALAuthenticationProvider*) getGraphAuthProvider;
    ```

1. Откройте **AuthenticationManager. m** и добавьте указанную ниже функцию в `AuthenticationManager` класс.
